...
export const DesktopSidebar = ({
  ...
  typeof motion.div
>) => {
  const { open, setOpen, animate } =
    useSidebar();
  return (
    <>
      <motion.div
        className={cn(
          "h-full px-3 py-4 hidden  md:flex md:flex-col dark:bg-neutral-900 bg-gray-50 w-[300px] shrink-0",
          className,
        )}
        animate={{
          width: animate
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className={cn(
                "fixed h-full w-full inset-0 bg-zinc-50 dark:bg-zinc-900 p-10 z-[100] flex flex-col justify-between",
                className,
              )}
            >
              <div
